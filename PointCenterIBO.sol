...
// File: @befswap/beef-swap-lib/contracts/math/SafeMath.sol
...
// File: @befswap/beef-swap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: @befswap/beef-swap-lib/contracts/utils/Address.sol
...
// File: @befswap/beef-swap-lib/contracts/token/BEP20/SafeBEP20.sol
...
// File: @befswap/beef-swap-lib/contracts/utils/ReentrancyGuard.sol
...
// File: contracts/utils/IBO.sol

pragma solidity 0.6.12;

contract IBO is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // get the amount of IBO token you will get
...
// File: contracts/interfaces/IBeefswapProfile.sol

pragma solidity ^0.6.0;

/** @title IBeefswapProfile.
 */
interface IBeefswapProfile {
...
// File: contracts/PointCenterIBO.sol

pragma solidity ^0.6.12;

/** @title PointCenterIBO.
@dev It is a contract for users to collect points
for IBOs they participated in.
*/
contract PointCenterIBO is Ownable {
    IBeefswapProfile beefswapProfile;

    uint256 public maxViewLength;

    mapping(address => IBOs) public ibos;
    ...
    constructor(address _beefswapProfileAddress, uint256 _maxViewLength) public {
        beefswapProfile = IBeefswapProfile(_beefswapProfileAddress);
    ...
    function getPoints(address _contractAddress) external {
        ...
        // 1. Check if IBO is valid
        require(
            (ibos[_contractAddress].campaignId > 0) &&
                (ibos[_contractAddress].numberPoints > 0),
            "not valid"
        );
        ...
        require(
            ...
            "has claimed for this IBO"
        );

        // 3. Check if he is active
        bool isUserActive = beefswapProfile.getUserStatus(senderAddress);
        require(isUserActive, "not active");

        // 4. Check if he can claim
        IBO currentIfo = IBO(_contractAddress);
        ...
        require(
            amountUser > ibos[_contractAddress].thresholdToClaim,
            "too small"
        );
        ...
        beefswapProfile.increaseUserPoints(
            senderAddress,
            ibos[_contractAddress].numberPoints,
            ibos[_contractAddress].campaignId
        );
    }

    function addIBOAddress(
        ...
        returns (bool)
    {
        ...
        if (
            ...
            // 1. Check if user has claimed funds from IBO AND match threshold
            return false;
        } else if (_users[_userAddress][_contractAddress]) {
            // 2. Check if user has already claimed points for this IBO
            return false;
        } else if (
            // 3. Check if a campaignId AND numberPoints were set
            (ibos[_contractAddress].campaignId < 1) ||
            (ibos[_contractAddress].numberPoints < 1)
        ) {
            return false;
        } else {
            // 4. Can claim
