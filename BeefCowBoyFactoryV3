...
// File: contracts/CowboyMintingStation.sol

pragma solidity ^0.6.0;

/** @title CowboyyMintingStation.
@dev It is a contract that allow different factories to mint
Beefswap Collectibles/Cowboy.
*/

contract CowboyMintingStation is AccessControl {
    BeefCowBoy public beefCowboy;
    ...
    constructor(BeefCowBoy _beefCowboy) public {
        beefCowboy = _beefCowboy;
    ...
    function mintCollectible(
        ...
        uint256 tokenId =
            beefCowboy.mint(_tokenReceiver, _tokenURI, _cowboyId);
        return tokenId;
    }

    /**
     * @dev Set up names for cowboy.
     * Only the main admins can set it.
     */
    function setCowboyName(uint8 _cowboyId, string calldata _cowboyName)
        external
        onlyOwner
    {
        beefCowboy.setCowboyName(_cowboyId, _cowboyName);
    }
...
// File: contracts/CowboyFactoryV3.sol

pragma solidity ^0.6.12;

contract CowboyFactoryV3 is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    CowboyFactoryV2 public cowboyFactoryV2;
    CowboyMintingStation public cowboyMintingStation;
    ...
    constructor(
        CowboyFactoryV2 _cowboyFactoryV2,
        CowboyMintingStation _cowboyMintingStation,
        IBEP20 _beefToken,
        uint256 _tokenPrice,
        string memory _ipfsHash,
        uint256 _startBlockNumber
    ...
    /**
     * @dev Mint NFTs from the CowboyMintingStation contract.
     * Users can specify what cowboyId they want to mint. Users can claim once.
     */
    function mintNFT(uint8 _cowboyId) external {
        address senderAddress = _msgSender();

        bool hasClaimedV2 = cowboyFactoryV2.hasClaimed(senderAddress);
        ...
        // Send BEEF tokens to this contract
        beefToken.safeTransferFrom(senderAddress, address(this), tokenPrice);

        string memory tokenURI = cowboyIdURIs[_cowboyId];

        uint256 tokenId =
            cowboyMintingStation.mintCollectible(
                senderAddress,
                tokenURI,
                _cowboyId
            );

        emit CowboyMint(senderAddress, tokenId, _cowboyId);
    }
    ...
    function canMint(address userAddress) external view returns (bool) {
        if (
            ...
            (cowboyFactoryV2.hasClaimed(userAddress))
        ) {
            return false;
        } else {
            return true;
