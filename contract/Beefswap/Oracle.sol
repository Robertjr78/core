...
library BeefswapOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IBeefswapPair(pair).price0CumulativeLast();
        price1Cumulative = IBeefswapPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IBeefswapPair(pair).getReserves();
...
contract Oracle {
    ...
    function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'BeefswappFactory: IDENTICAL_ADDRESSES');
    ...
    function update(address tokenA, address tokenB) external {
        require(msg.sender == priceUpdater, 'BeefswapOracle: Price can update only price updater address');
        address pair = pairFor(tokenA, tokenB);

        Observation storage observation = pairObservations[pair];
        uint timeElapsed = block.timestamp - observation.timestamp;
        require(timeElapsed >= CYCLE, 'BeefswapOracle: PERIOD_NOT_ELAPSED');
        (uint price0Cumulative, uint price1Cumulative,) = BeefswapOracleLibrary.currentCumulativePrices(pair);
