...
contract SmartChefNFT is Ownable, ReentrancyGuard {
    ...
    uint public totalBBSupply;
    uint public lastRewardBlock;
    address[] public listRewardTokens;
    IBeefswapNFT public nftToken;
    ...
    event StakeTokens(address indexed user, uint amountBB, uint[] tokensId);
    event UnstakeToken(address indexed user, uint amountBB, uint[] tokensId);
    event EmergencyWithdraw(address indexed user, uint tokenCount);

    constructor(IBeefswapNFT _nftToken) {
        nftToken = _nftToken;
    }
    ...
    function getUserStakedRbAmount(address _user) public view returns(uint){
        return userInfo[_user].stakedBbAmount;
    }
    ...
    function pendingReward(address _user) external view returns (address[] memory, uint[] memory) {
        ...
        if(user.stakedBbAmount == 0){
            return (listRewardTokens, rewards);
        }
        uint _totalBBSupply = totalBBSupply;
        ...
        for(uint i = 0; i < listRewardTokens.length; i++){
            ...
            if (_multiplier != 0 && _totalBBSupply != 0 && curRewardToken.enabled == true) {
                uint curMultiplier;
                ...
                (curMultiplier * curRewardToken.rewardPerBlock * 1e12 / _totalBBSupply);
            } else {
                _accTokenPerShare = curRewardToken.accTokenPerShare;
            }
            rewards[i] = (user.stakedBbAmount * _accTokenPerShare / 1e12) - rewardDebt[_user][curToken];
        }
        return (listRewardTokens, rewards);
    }
    ...
    function updatePool() public {
        ...
        uint _totalBBSupply = totalBBSupply; //Gas safe
        ...
        for(uint i = 0; i < listRewardTokens.length; i++){
            ...
            if(curRewardToken.enabled == false || curRewardToken.startBlock >= block.number){
                ...
                rewardTokens[curToken].accTokenPerShare += (tokenReward * 1e12) / _totalBBSupply;
            }
        }
    }
    ...
    function _updateRewardDebt(address _user) internal {
        for(uint i = 0; i < listRewardTokens.length; i++){
            rewardDebt[_user][listRewardTokens[i]] = userInfo[_user].stakedBbAmount * rewardTokens[listRewardTokens[i]].accTokenPerShare / 1e12;
        }
    }
    ...
    function _withdrawReward() internal {
        ...
        for(uint i = 0; i < _listRewardTokens.length; i++){
            ...
            uint pending = user.stakedBbAmount * curRewardToken.accTokenPerShare / 1e12 - rewardDebt[msg.sender][_listRewardTokens[i]];
            if(pending > 0){
                curRewardToken.rewardsForWithdrawal -= pending;
                rewardDebt[msg.sender][_listRewardTokens[i]] = user.stakedBbAmount * curRewardToken.accTokenPerShare / 1e12;
    ...
    function stake(uint[] calldata tokensId) public nonReentrant {
        _withdrawReward();
        uint depositedBigBoost = 0;
        for(uint i = 0; i < tokensId.length; i++){
            (address tokenOwner, bool stakeFreeze, uint bigBoost) = nftToken.getInfoForStaking(tokensId[i]);
            ...
            depositedBigBoost += bigBoost;
            userInfo[msg.sender].stakedTokensId.push(tokensId[i]);
        }
        if(depositedBigBoost > 0){
            userInfo[msg.sender].stakedBbAmount += depositedBigBoost;
            totalBBSupply += depositedBigBoost;
        }
        _updateRewardDebt(msg.sender);
        emit StakeTokens(msg.sender, depositedBigBoost, tokensId);
    }
    ...
    function unstake(uint[] calldata tokensId) public nonReentrant {
        ...
        for(uint i = 0; i < tokensId.length; i++){
            ...
            for(uint j = 0; j < user.stakedTokensId.length; j++){
                if(tokensId[i] == user.stakedTokensId[j]){
                    ...
                    withdrawalBBAmount += nftToken.getBB(tokensId[i]);
        ...
        if(withdrawalBBAmount > 0){
            user.stakedBbAmount -= withdrawalBBAmount;
            totalBBSupply -= withdrawalBBAmount;
            _updateRewardDebt(msg.sender);
        }
        emit UnstakeToken(msg.sender, withdrawalBBAmount, tokensId);
    }

    // Withdraw without caring about rewards. EMERGENCY ONLY.
    function emergencyUnstake() public {
        uint[] memory tokensId = userInfo[msg.sender].stakedTokensId;
        totalBBSupply -= userInfo[msg.sender].stakedBbAmount;
