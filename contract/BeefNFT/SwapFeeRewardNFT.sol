...
contract SwapFeeRewardWithBB is Ownable, ReentrancyGuard {
    ...
    uint public maxAccruedBBInPhase = 5000 ether;
    ...
    //percent of distribution between feeReward and bigBoost [0, 90] 0 => 90% feeReward and 10% bigBoost; 90 => 100% bigBoost
    ...
    event NewPhase(uint);
    event NewPhaseBB(uint);
    event NewAuction(address);
    event NewBeefswapNFT(IBeefswapNFT);
    ...
    constructor(
        ...
        beefswapNFT = _beefswapNFT;
        targetBBToken = _targetBBToken;
    }
    ...
    function setPhaseBB(uint _newPhase) public onlyOwner returns (bool){
        currentPhaseBB = _newPhase;
        //BNF-01, SFR-01
        emit NewPhaseBB(_newPhase);
        return true;
    }
    ...
    )
    {
        ...
        if (pool.pair != pair || pool.enabled == false || !isWhitelist(input) || !isWhitelist(output)) {
            ...
            (uint feeAmount, uint bbAmount) = calcAmounts(amount, account);
            uint256 fee = feeAmount.div(pairFee);
            uint256 quantity = getQuantity(output, fee, targetToken);
            feeReturnInBEEF = quantity.mul(pool.percentReward).div(100);
            bigBoostAccrue = getQuantity(output, bbAmount.div(bbWagerOnSwap), targetBBToken);
            feeReturnInUSD = getQuantity(targetToken, feeReturnInBEEF, targetBBToken);
        }
    }
    ...
    function swap(address account, address input, address output, uint256 amount) public onlyRouter returns (bool) {
        ...
        (uint feeAmount, uint bbAmount) = calcAmounts(amount, account);
        uint256 fee = feeAmount.div(pairFee);
        bbAmount = bbAmount.div(bbWagerOnSwap);
        //SFR-05
        _accrueBB(account, output, bbAmount);
    ...
    function calcAmounts(uint amount, address account) internal view returns(uint feeAmount, uint bbAmount){
        feeAmount = amount.mul(defaultFeeDistribution.sub(feeDistribution[account])).div(100);
        bbAmount = amount.sub(feeAmount);
    }

    function accrueBBFromMarket(address account, address fromToken, uint amount) public onlyMarket {
        //SFR-05
        amount = amount.mul(bbPercentMarket).div(10000);
        _accrueBB(account, fromToken, amount);
    }

    function accrueBBFromAuction(address account, address fromToken, uint amount) public onlyAuction {
        //SFR-05
        amount = amount.mul(bbPercentAuction).div(10000);
        _accrueBB(account, fromToken, amount);
    }

    //SFR-05
    function _accrueBB(address account, address output, uint amount) private {
        uint quantity = getQuantity(output, amount, targetBBToken);
        if (quantity > 0) {
            //SFR-06
            totalAccruedBB = totalAccruedBB.add(quantity);
            if(totalAccruedBB <= currentPhaseBB.mul(maxAccruedBBInPhase)){
                beefswapNFT.accrueBB(account, quantity);
            }
        }
    }
    ...
    function withdraw(uint8 v, bytes32 r, bytes32 s) public nonReentrant returns (bool){
        ...
        if (balance > 0) {
            ...
            //SFR-04
            if(beefToken.transfer(msg.sender, balance)){
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
        ...
        } else if (IBeefswapFactory(factory).getPair(outputToken, anchorToken) != address(0) && checkPairExist(outputToken, anchorToken)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (IBeefswapFactory(factory).getPair(outputToken, intermediate) != address(0) && IBeefswapFactory(factory).getPair(intermediate, anchorToken) != address(0) && checkPairExist(intermediate, anchorToken)) {
    ...
    function setBeefswapNFT(IBeefswapNFT _beefswapNFT) public onlyOwner {
        require(address(_beefswapNFT) != address(0), "SwapMining: new beefswapNFT is the zero address");
        beefswapNFT = _beefswapNFT;
        //BNF-01, SFR-01
        emit NewBeefswapNFT(_beefswapNFT);
    }
    ...
    function setBigBoostReward(uint _bbWagerOnSwap, uint _percentMarket, uint _percentAuction) public onlyOwner {
        bbWagerOnSwap = _bbWagerOnSwap;
        bbPercentMarket = _percentMarket;
        bbPercentAuction = _percentAuction;
    }
