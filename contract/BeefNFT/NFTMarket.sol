...
contract Market is ReentrancyGuard, Ownable, Pausable {
    ...
    uint8 public maxUserTokenOnSellToReward = 3; //max count sell offers of nftForAccrualBB on which Rb accrual
    uint256 rewardDistributionSeller = 50; //Distribution reward between seller and buyer. Base 100
    address public treasuryAddress;
    ISwapFeeRewardWithBB feeRewardBB;
    ISmartChefMarket smartChefMarket;
    bool feeRewardBBIsEnabled = false; // Enable/disable accrue BB reward for trade NFT tokens from nftForAccrualBB list
    ...
    mapping(address => bool) public nftForAccrualBB; //add tokens on which BigBoost is accrual
    ...
    constructor(
        ...
        ISwapFeeRewardWithBB _feeRewardBB
    ) {
        //NFT-01
        require(_treasuryAddress != address(0), "Address cant be zero");
        treasuryAddress = _treasuryAddress;
        feeRewardBB = _feeRewardBB;
        feeRewardBBIsEnabled = false;
    ...
    function enableBBFeeReward() public onlyOwner {
        feeRewardBBIsEnabled = true;
    }

    function disableBBFeeReward() public onlyOwner {
        feeRewardBBIsEnabled = false;
    }
    ...
    function addNftForAccrualBB(address _nft) public onlyOwner {
        require(_nft != address(0), "Address cant be zero");
        nftForAccrualBB[_nft] = true;
        emit NFTAccrualListUpdate(_nft, true);
    }

    function delNftForAccrualBB(address _nft) public onlyOwner {
        require(_nft != address(0), "Address cant be zero");
        delete nftForAccrualBB[_nft];
    ...
    function setFeeRewardBB(ISwapFeeRewardWithBB _feeRewardBB)
        public
        onlyOwner
    {
        require(address(_feeRewardBB) != address(0), "Address cant be 0");
        feeRewardBB = _feeRewardBB;
    }
    ...
    function placementRewardQualifier(
        ...
        //Check if nft token in nftForAccrualBB list and accrue reward enable
        if (!nftForAccrualBB[nftToken] || !placementRewardEnabled) return;
    ...
    function _distributePayment(Offer memory _offer) internal {
        ...
        if (feeRewardBBIsEnabled && nftForAccrualBB[address(_offer.nft)]) {
            feeRewardBB.accrueBBFromMarket(
                seller,
