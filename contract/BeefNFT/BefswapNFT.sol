...
contract BeefswapNFT is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    bytes32 public constant BB_SETTER_ROLE = keccak256("BB_SETTER");
    uint public constant MAX_ARRAY_LENGTH_PER_REQUEST = 30;

    string private _internalBaseURI;
    uint private _initialBigBoost;
    ...
    mapping(address => mapping(uint => uint)) private _bigBoost;
    mapping(uint => uint) private _bigBoostTotalAmounts;
    ...
    //BNF-01, SFR-01
    event Initialize(string baseURI, uint initialBigBoost, uint burnBBPeriod);
    event TokenMint(address indexed to, uint indexed tokenId, uint level, uint bigBoost);

    function initialize(
        string memory baseURI,
        uint initialBigBoost,
        uint burnBBPeriod
    ) public initializer {
        __ERC721_init("BigBoost", "BB");
        ...
        _initialBigBoost = initialBigBoost;
        ...
        //BNF-01, SFR-01
        emit Initialize(baseURI, initialBigBoost, burnBBPeriod);
    }
    ...
    function getBB(uint tokenId) external view returns(uint){
        return _tokens[tokenId].bigBoost;
    }
    ...
    function getInfoForStaking(uint tokenId) external view returns(
        ...
        bigBoost = _tokens[tokenId].bigBoost;
    ...
    function accrueBB(address user, uint amount) external onlyRole(BB_SETTER_ROLE) {
        ...
        increaseBigBoost(user, curDay, amount);
        emit BBAccrued(user, _bigBoost[user][curDay]);
    }
    ...
    function getBbBalanceByDays(address user, uint dayCount) public view returns(uint[] memory){
        ...
        for(uint i = 0; i < dayCount; i++){
            balance[i] = _bigBoost[user][(block.timestamp - i * 1 days)/86400];
        }
        return balance;
    }
    ...
    function getBbTotalAmount(uint period) public view returns(uint amount){
        for(uint i = 0; i <= period; i++){
            amount += _bigBoostTotalAmounts[(block.timestamp - i * 1 days)/86400];
        }
        return amount;
    }
    ...
    function mint(address to) public onlyRole(TOKEN_MINTER_ROLE) nonReentrant {
        ...
        _tokens[tokenId].bigBoost = _initialBigBoost;
    ...
    //BNF-02, SCN-01, SFR-02
    function launchpadMint(address to, uint level, uint bigBoost) public onlyRole(LAUNCHPAD_TOKEN_MINTER) nonReentrant {
        require(to != address(0), "Address can not be zero");
        require(_bbTable[level] >= bigBoost, "BB Value out of limit");
        _lastTokenId +=1;
        uint tokenId = _lastTokenId;
        _tokens[tokenId].bigBoost = bigBoost;
        _tokens[tokenId].createTimestamp = block.timestamp;
    ...
    function _safeMint(address to, uint256 tokenId) internal override {
        ...
        emit TokenMint(to, tokenId, _tokens[tokenId].level, _tokens[tokenId].bigBoost);
    }
    ...
    function _getBbBalance(address user) internal view returns(uint balance){
        for(uint i = 0; i <= _burnBBPeriod; i++){
            balance += _bigBoost[user][(block.timestamp - i * 1 days)/86400];
        }
        return balance;
    }

    function _remainBBToMaxLevel(uint tokenId) internal view returns(uint) {
        return _bbTable[uint(_tokens[tokenId].level)] - _tokens[tokenId].bigBoost;
    }
    ...
    function _sendBBToToken(uint[] memory tokenId, uint[] memory amount) internal {
        ...
        for(uint i = 0; i < tokenId.length; i++){
            ...
            while(calcAmount > 0 || period > 0){
                ...
                currentBB = _bigBoost[msg.sender][curDay];
                if(currentBB == 0) {
                    period--;
                    continue;
                }
                if(calcAmount > currentBB){
                    ...
                    _bigBoostTotalAmounts[curDay] -= currentBB;
                    delete _bigBoost[msg.sender][curDay];

                } else {
                    decreaseBigBoost(msg.sender, curDay, calcAmount);
                    calcAmount = 0;
                    break;
                }
                period--;
    ...
    function _mintLevelUp(uint level, uint[] memory tokenId) private {
        ...
        for(uint i = 0; i <tokenId.length; i++){
            require(ownerOf(tokenId[i]) == msg.sender, "Not owner of token");
            newBigBoost += _tokens[tokenId[i]].bigBoost;
            _burn(tokenId[i]);
        }
        newBigBoost = newBigBoost + newBigBoost * _levelUpPercent / 100;
        _lastTokenId +=1;
        uint newTokenId = _lastTokenId;
        _tokens[newTokenId].bigBoost = newBigBoost;
        _tokens[newTokenId].createTimestamp = block.timestamp;
        _tokens[newTokenId].level = level;
        _safeMint(msg.sender, newTokenId);
    }

    function increaseBigBoost(address user, uint day, uint amount) private {
        _bigBoost[user][day] += amount;
        _bigBoostTotalAmounts[day] += amount;
    }

    function decreaseBigBoost(address user, uint day, uint amount) private {
        require(_bigBoost[user][day] >= amount && _bigBoostTotalAmounts[day] >= amount, "Wrong amount");
        _bigBoost[user][day] -= amount;
        _bigBoostTotalAmounts[day] -= amount;
    }
